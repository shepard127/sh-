...
int main(int argc, string argv[])
{
    ...
    d = atoi(argv[1]);
    if (d < DIM_MIN || d > DIM_MAX)             // проверяем что там пользователь ввел
    {
...
void init(void)
{
    ...
    // выполнится в самом конце
    if(row == d)
    {
        if((board[0][0] % 2) != 0)                                  // если наша матрица начинается на нечетный элемент - то 1 и 2 меняем местами
        {
            int replace = d - 1;
...
{
    // TODO
    /* 1 - определить текущую позицию tile */
    /* 2 - если в диапазоне 1-й клетки от tile, есть 0 - то меняем tile с нолем местами */
